...
SQL.Relation.prototype.init = function(owner, row1, row2, id) {	
	...
	if (this.owner.vector) {
		...
		/**
		 * Add a click event to relation to enable removal of relation.
		 */
		OZ.Event.add(path, "click", this.bind(this.selectRelation));
...
SQL.Relation.prototype.selectRelation = function(e) {
	this.owner.tableManager.toggleRemoveConnection(true);
	window.selectedConnection = new Array();
	window.selectedConnection.push(this);
}
...
SQL.IO.prototype.move=function()
{
...
SQL.TableManager.prototype.init = function(owner) {
	...
	var ids = ["addtable","removetable","cleartables","edittable","copyblock","foreignconnect","foreigndisconnect"];
	for (var i=0;i<ids.length;i++) {
		var id = ids[i];
		var elm = OZ.$(id);
		this.dom[id] = elm;
...
//		this.dom.foreigncreate.disabled = false;//!(this.selected.isUnique());
		this.dom.foreignconnect.disabled = false;// !(this.selected.isUnique());
...
SQL.KeyManager.prototype.right = function(e) { /* remove field from index */
	...
	this.switchTo(this.dom.list.edIndex);
}
...
SQL.Window.prototype.init = function(owner) {
	...
	/**
	 * Added for mousemove of whiteboard
	 */
	OZ.Event.add(window, "mousemove", this.mousemove);
...
SQL.Window.prototype.mousemove= function(e) {
	if (!e) var e = window.event;
	if (e.pageX || e.pageY)     {
		window.globalmouse_x = e.pageX;
		window.globalmouse_y = e.pageY;
	}
	else if (e.clientX || e.clientY)     {
		window.globalmouse_x = e.clientX + document.body.scrollLeft
		+ document.documentElement.scrollLeft;
		window.globalmouse_y = e.clientY + document.body.scrollTop
		+ document.documentElement.scrollTop;
	}
	window.status="x: "+window.globalmouse_x+" y: "+window.globalmouse_y;
...
//	}
	...
	//alert(completeXML.toString());
	/*var dom = parseXml(completeXML);
